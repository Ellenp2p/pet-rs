...
impl RateLimiter {
    ...
    pub fn check_request(&mut self, tokens: u32) -> Result<(), AIError> {
        ...
        if let Some(rpm) = self.config.requests_per_minute {
            ...
            let recent = self.request_history.iter().filter(|&&t| t > minute_ago).count();
            if recent >= rpm as usize {
                return Err(AIError::RateLimited(format!("每分钟请求限制已达 {} 次", rpm)));
            }
        }
        ...
        if let Some(rph) = self.config.requests_per_hour {
            ...
            let recent = self.request_history.iter().filter(|&&t| t > hour_ago).count();
            if recent >= rph as usize {
                return Err(AIError::RateLimited(format!("每小时请求限制已达 {} 次", rph)));
            }
        }
        ...
        if let Some(tpm) = self.config.tokens_per_minute {
            let minute_ago = now - Duration::from_secs(60);
            let recent_tokens: u32 = self.token_history.iter()
                .filter(|(t, _)| *t > minute_ago)
                .map(|(_, tokens)| tokens)
                .sum();
            if recent_tokens + tokens > tpm {
                return Err(AIError::RateLimited(format!("每分钟 Token 限制已达 {}", tpm)));
            }
        }
        ...
        if let Some(tph) = self.config.tokens_per_hour {
            ...
            if recent_tokens + tokens > tph {
                return Err(AIError::RateLimited(format!("每小时 Token 限制已达 {}", tph)));
            }
        }
    ...
    pub fn get_status(&self) -> (usize, usize, u32, u32) {
        ...
        let req_min = self.request_history.iter().filter(|&&t| t > minute_ago).count();
        let req_hour = self.request_history.iter().filter(|&&t| t > hour_ago).count();
        let tok_min: u32 = self.token_history.iter().filter(|(t, _)| *t > minute_ago).map(|(_, t)| t).sum();
        let tok_hour: u32 = self.token_history.iter().filter(|(t, _)| *t > hour_ago).map(|(_, t)| t).sum();
